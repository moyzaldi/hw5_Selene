...
def test_type_registration_form(browser_settings):
    ...
    browser.element(("[class='modal-body']")).should(have.text(userEmail))
    browser.element(("[class='modal-body']")).should(have.text(genter))
    browser.element(("[class='modal-body']")).should(have.text(userNumber))
    browser.element(("[class='table-responsive']")).should(have.text(f'{day} {month},{year}'))
    # browser.element(("[class='modal-body']")).should(have.text(subjects))
    browser.element(("[class='modal-body']")).should(have.text(hobbies))
    browser.element(("[class='modal-body']")).should(have.text(images))
    browser.element(("[class='modal-body']")).should(have.text(currentAddress))
